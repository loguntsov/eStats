...
handle_click(Click, Report) ->
  ...
  estats_report:counter_inc(Report, [ Click#click_info.date, Click#click_info.offer_id ] , [ Click#click_info.affiliate_id, Hour ]),
  estats_report:counter_inc(Report, [ Click#click_info.date, Click#click_info.offer_id ] , [ 0 , Hour ]),
...
handle_report({affiliates_of_offer, Period, Offer }, Report) when is_list(Period) ->
  {ok, 0,
    [ X || X <- lists:usort(
      [ Affiliate || [ _, _, Affiliate ] <- estats_report:index_get_all(Report, [Period, Offer], [])]
    ), X > 0]
  };
...
handle_report({affiliates_day_count, Period, { Offer, Affiliate } }, Report) when is_list(Period) ->
  {ok, 2,
    [ { estats_report:subkey_swap(Key, [2, 1, 3]), Value } || {Key, Value} <-
      estats_report:counters_list_get(Report,
        estats_report:subkey_list(
